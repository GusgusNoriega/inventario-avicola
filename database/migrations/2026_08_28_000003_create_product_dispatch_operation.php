...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('pesadas_despacho_productos', function (Blueprint $table): void {
            ...
            $table->foreignId('ticket_despacho_producto_id')
                ->constrained('tickets_despacho_productos')
                ->cascadeOnDelete();
            ...
            $table->foreignId('variacion_producto_despacho_id')
                ->nullable()
                ->constrained('variaciones_producto_despacho')
                ->restrictOnDelete();
            $table->foreignId('lectura_balanza_id')
                ->nullable()
                ->unique()
                ->constrained('lecturas_balanza')
                ->nullOnDelete();
    ...
    public function down(): void
    {
        ...
        Schema::table('comprobante_detalles', function (Blueprint $table): void {
            $table->dropConstrainedForeignId('variacion_producto_despacho_id');
            $table->dropConstrainedForeignId('producto_despacho_id');
            $table->dropColumn([
                'cantidad_unidades',
                'modo_precio',
                'precio_unitario',
